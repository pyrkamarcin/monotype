...
namespace Monotype\Bundle\DeamonBundle\System;

use Monotype\Bundle\DaemonBundle\System\Options;
use Monotype\Bundle\DaemonBundle\System\OS;

/**
 * Class Daemon
 * @package Monotype\Bundle\DeamonBundle\System
 */
class Daemon
{
    ...
    static public function start()
    {
        ...
        if (self::opt('usePEAR')) {
            ...
            if (false === class_exists('PEAR', true)) {
                $msg = 'PEAR not found. Install PEAR or run with option: ' . 'usePEAR = false';
    ...
    )
    {
        ...
        if (null === self::opt('logVerbosity') || false === self::opt('logVerbosity')
        ) {
        ...
        if (self::opt('usePEARLogInstance') !== false) {
            self::opt('usePEARLogInstance')
                ->log($str . $log_tail, $level);
        ...
        if (!self::opt('logLocation')) {
            throw new Exception('Either use PEAR Log or specify ' . 'a logLocation');
        }
        ...
        if (!is_writable(self::opt('logLocation')) && $non_debug && !$log_echoed
        ) {
            echo $log_line . "\n";
            $log_echoed = true;
            $log_succeeded = false;
