...
namespace Monotype\Bundle\DaemonBundle\System;
...
class Options
{
        ...
        foreach ($this->_definitions as $name => $definition) {
            ...
            if (!isset($this->_options[$name])) {
                if (!$this->_setDefault($name) && !$premature && @$definition['required']
                    ...
                    $this->errors[] = 'Required option: ' . $name . ' not set. No default value available either.';
                    return false;
                }
            }
    ...
    protected function _validate($name, $value, &$reason = "")
    {
                ...
                foreach ($sub_types as $type_b) {
                    ...
                    switch ($type_a) {
                        ...
                        case "number":
                            ...
                            break;
                        default:
                            $this->errors[] = "Type " . $type_a . " not defined";
                            break;
                    }
                }
            }
        ...
        if (!$type_valid) {
            $reason = "Option " . $name . " does not match type: " . $definition["type"] . "";
        }
